...
// Valid phone check
const isValidPhoneNumber = (number) => /^[1-9][0-9]{7,14}$/.test(normalizeNumber(number));
...
async (conn, mek, m, { from, q, reply }) => {
    try {
        ...
        const sender = normalizeNumber(m.sender.split('@')[0]);

        if (sender !== ownerNumber) {
            return reply(`😤 YOU VILE IMPOSTOR! Only owner (${ownerNumber}) can use this power!`);
        }

        if (!q && !m.mentionedJid.length && !m.quoted) {
            return reply(`⚠️ Missing target! Use: .crash 923xxxxxxxxx or reply/tag a user.`);
        }

        let target = m.mentionedJid[0] || (m.quoted && m.quoted.sender) || q;
        let targetNumber = normalizeNumber(target.includes('@') ? target.split('@')[0] : target);

        if (!isValidPhoneNumber(targetNumber)) {
            return reply(`❌ Invalid number! Use correct format like 923xxxxxxxxx`);
        }
